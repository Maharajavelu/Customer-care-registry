    ...
    if request.method == 'POST':
        ...
        if account:
            ...
            TEXT1 = '''<!DOCTYPE html>
                        <html>
                        <body>
                        <h3> Account creation in customer care registry was successful.<br>
                        For raising Complaints, login with your username and password.<br><br>
                        Thank You</h3>
                        </body></html>'''
...
def agcomplainthist(): 
    ...
    while comp != False:
        data.append(comp)
        comp = ibm_db.fetch_assoc(stmt1)
    return render_template('agentcomphist.html', complaints = data)
...
def solved(no):
    ...
    ibm_db.execute(stmt)
    stmt1 = ibm_db.prepare(conn, 'SELECT email FROM complaintdetails  WHERE uid = ?')
    ibm_db.bind_param(stmt1, 1, i)  
    ibm_db.execute(stmt1)
    acc = ibm_db.fetch_tuple(stmt1)
    email = acc[6]
    print(email)
    TEXT = """<!DOCTYPE html>
                <html>
                <body>
                    <h3>Your Complaint has been solved.</h3>
                </body>
                </html>"""
    usermail(TEXT,email)
